...
export const startMatch = async (context: PostMatchContext) => {
  try {
    ...
    let comprobarEquipo1: PartidoSchema | undefined = await PartidoCollection.findOne({
      nombreEquipo1: value.nombreEquipo1
    });

    let comprobarEquipo2: PartidoSchema | undefined = await PartidoCollection.findOne({
