...
export const Mutation = {
    setMatchData: async (_: unknown, args: {id: string, result: string, minute: number, ended: boolean}, ): Promise<MatchSchema> => {
        ...
        try {
            ...
            const matchExiste: MatchSchema | undefined = await MatchCollection.findOne({
            ...
            if (matchExiste?.minutoJuego > args.minute) {
    ...
    startMatch: async (_: unknown, args: {team1: string, team2: string}, ): Promise<MatchSchema> => {
        try {
            let comprobarEquipo1: MatchSchema | undefined = await MatchCollection.findOne({
                nombreEquipo1: args.team1
              });
          
              let comprobarEquipo2: MatchSchema | undefined = await MatchCollection.findOne({
